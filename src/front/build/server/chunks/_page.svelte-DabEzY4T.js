...
//#region src/routes/beneficial-ownership-merchant-fleets/+page.svelte
function _page($$renderer, $$props) {
	$$renderer.component(($$renderer) => {
		...
		let newTons = "";
		let newPercent = "";
		$$renderer.push(`<div class="container" style="max-width: 1100px;"><h1>🚢 Beneficial Ownership Merchant Fleets</h1> `);
		$$renderer.push("<!--[-1-->");
		$$renderer.push(`<!--]--> `);
		$$renderer.push("<!--[-1-->");
		...
		if (fleets.length === 0) {
			$$renderer.push("<!--[0-->");
			$$renderer.push(`<tr><td colspan="6" style="text-align: center; padding: 20px;">No hay datos disponibles en la base de datos.</td></tr>`);
		} else {
			$$renderer.push("<!--[-1-->");
			$$renderer.push(`<!--[-->`);
			...
			for (let $$index = 0, $$length = each_array.length; $$index < $$length; $$index++) {
				...
				$$renderer.push(`<tr style="border-bottom: 1px solid var(--border-color);"><td style="padding: 12px;">${escape_html(fleet.year)}</td><td style="padding: 12px;">${escape_html(fleet.flag_of_registration_label)}</td><td style="padding: 12px;">${escape_html(fleet.beneficial_ownership_label)}</td><td style="padding: 12px; font-weight: bold;">${escape_html(fleet.number_of_ships)}</td><td style="padding: 12px;">${escape_html(fleet.dead_weight_tons)}</td><td style="padding: 12px;">${escape_html(fleet.percentage_of_total_fleet)}%</td></tr>`);
			}
			$$renderer.push(`<!--]-->`);
		}
...
//# sourceMappingURL=_page.svelte-DabEzY4T.js.map
